		...
		public AutoMapperProfiles() {
			...
			CreateMap<Pelicula, PeliculaDTO>()
				.ForMember( dto => dto.Cines,
					ent => ent.MapFrom( prop => prop.SalasCine.Select( s => s.Cine ) ) )
				.ForMember( dto => dto.Actores,
					ent => ent.MapFrom( prop => prop.PeliculasActores.Select( pa => pa.Actor ) ) );

			// Con projectTo
			//CreateMap<Pelicula, PeliculaDTO>()
			//	.ForMember(dto => dto.Generos, 
			//		ent => ent.MapFrom(prop => prop.Generos.OrderByDescending(g => g.Nombre)))
			//	.ForMember( dto => dto.Cines,
			//		ent => ent.MapFrom( prop => prop.SalasCine.Select( s => s.Cine ) ) )
			//	.ForMember( dto => dto.Actores,
			//		ent => ent.MapFrom( prop => prop.PeliculasActores.Select( pa => pa.Actor ) ) );
